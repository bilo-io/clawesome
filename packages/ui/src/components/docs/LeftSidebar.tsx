...
export function LeftSidebar({ 
  ...
  return (
    <>
      ...
      <aside className={cn(
        "fixed top-0 md:top-16 left-0 z-40 w-64 h-full md:h-[calc(100vh-4rem)] border-r transition-all duration-300 overflow-y-auto no-scrollbar",
        ...
        <div className="p-6 space-y-8">
          <div className={cn(
            "relative flex items-center p-2 rounded-xl transition-colors",
            theme === 'dark' ? 'bg-slate-900 border border-slate-800' : 'bg-white border border-slate-200 shadow-sm'
          )}>
            <Search size={16} className={cn("ml-2", theme === 'dark' ? 'text-slate-500' : 'text-slate-400')} />
            <input 
              type="text" 
              placeholder="Search docs..." 
              className={cn(
                "w-full bg-transparent px-3 py-1 text-sm outline-none",
                theme === 'dark' ? 'text-white placeholder:text-slate-600' : 'text-black placeholder:text-slate-400'
              )}
            />
          </div>
                  ...
                  {section.links.map(link => (
                    <div key={link.href} className="space-y-1">
                      <Link 
                        href={link.href}
                        className={cn(
                          "flex items-center px-4 py-2 text-sm font-semibold rounded-lg transition-all",
                          isActive(link.href)
