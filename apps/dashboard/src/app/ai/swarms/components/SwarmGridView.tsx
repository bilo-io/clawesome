    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      <AnimatePresence mode="popLayout">
        <motion.div layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
          <InitializeCard label="Initialize Swarm" onClick={() => {}} viewMode="grid" />
        </motion.div>
        {swarms.map((ws) => {
          ...
          return (
            <motion.div 
              ...
              className={cn(
                "group transition-all cursor-pointer relative overflow-hidden border shadow-xl flex flex-col p-8 rounded-[40px]",
            ...
            >
              ...
              <div 
                ...
                className={cn(
                "rounded-full border-2 flex items-center justify-center transition-all absolute z-30 cursor-pointer w-7 h-7 top-6 right-6",
                selectedIds.includes(ws.id)
                ...
                {selectedIds.includes(ws.id) && <Check size={14} strokeWidth={4} />}
              </div>
  
              <div className="flex flex-col items-start">
                <div className={cn(
                  "rounded-2xl shadow-inner border transition-transform group-hover:scale-110 p-4 mb-8", 
                ...
                <div>
                  <h4 className={cn(
                    "font-black tracking-tight transition-colors text-xl", 
                  ...
                  <div className={cn("flex items-center gap-2 mt-2 group-hover:translate-x-1 transition-transform")}>
                    ...
                    <p className={cn(
                      "font-mono font-bold truncate tracking-tight uppercase text-[10px]", 
              ...
              <div className="flex flex-col gap-4 mt-8">
                 ...
                 <div className="flex items-center gap-8">
                    ...
                    <ChevronRight size={24} className={cn(
                      "transition-all absolute bottom-8 right-8 opacity-0 group-hover:opacity-100 group-hover:translate-x-2",
